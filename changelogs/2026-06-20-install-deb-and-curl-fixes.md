# 2026-06-20 — Install via .deb + curl reliability fixes
...
downloading individual files. Single download, no stdin issues, no partial
state on failure.
...
After creating `~/claude-docker/comfyui/user/default/workflows/`, the
postinst script copies the bundled `default.json` there (skipped if the file
already exists, so user-modified workflows are never overwritten).
