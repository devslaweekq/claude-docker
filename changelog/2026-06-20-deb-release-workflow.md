...
## Added
- `.github/workflows/deb-release.yml` — builds and publishes the `.deb` as a
  GitHub Release, running in parallel with `docker-publish.yml`.
  ...
  Runs `npm run build:deb`, creates release `v<version>` with the `.deb`
